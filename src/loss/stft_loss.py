...
def spectral_loss(p: torch.Tensor, t: torch.Tensor, floor: float = 0.3,
    ...
    """Amplitude + phase-alignment loss on interleaved re/im spectra,
    (T, 2F) or (B, T, 2F); unbatched input behaves as a batch of one.
...
def _selftest():
    ...
    # Unbatched (T, 2F) == batch of one.
    pb, tb = torch.randn(4, 9, 6), torch.randn(4, 9, 6)
