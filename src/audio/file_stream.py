...
class FileSource(StreamSource):
    def __init__(self, path, chunk_time: float, sr: int | None = None):
        raw, src_sr = sf.read(path, dtype='float32')
        self._wav = self._to_mono(raw)
        self._sr = src_sr if sr is None else sr
        if src_sr != self._sr:
            print(f"Warning: Resampling file from {src_sr} to {self._sr}", file=sys.stderr)
            self._wav = soxr.resample(self._wav, src_sr, self._sr, 'VHQ')
